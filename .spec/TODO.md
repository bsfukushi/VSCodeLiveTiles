...
> **現在: v0.15.0**（`src/VSCodeLiveTiles/VSCodeLiveTiles.csproj` の `<Version>` が正本）
...
### Step 7-8: タイトル順を「プロジェクト名 - ファイル名」に変更（v0.15.0）✅ 2026-07-31 リリース済み

**ゴール**（とも要望 2026-07-31）: VSCode 既定タイトルは `ファイル名 - フォルダ名` で、
ファイル未オープンのウィンドウだけフォルダ名が左端に来るため、タイル間で位置関係が揃わない。
帯の表示を `フォルダ名 - ファイル名` に並べ替え、どのタイルも左端がプロジェクト名になるようにする。

- [x] **表示文字列と照合キーの分離** — `CaptionText` は表示テキスト兼 CC 照合キーで、
      `CcStateTracker.Matches` が `EndsWith(プロジェクト名)` で判定している。
      **単純に順序を入れ替えると全バッジが消える（v0.14.1 と同じ症状）。**
      `_matchKey`（並べ替え前）を新設して `CaptionText` はそちらを返し、
      `_caption.Text` だけ並べ替える。**`CcStateTracker` は無改修**
- [x] `ThumbnailTile.ToProjectFirst()` — **最後の**区切りで割る（ファイル名側が ` - ` を
      含んでもプロジェクト名が壊れない）。半角ハイフン / em ダッシュ両対応。
      区切り無し・前後どちらかが空なら並べ替えない
- [x] `Bind` / `UpdateTitle` の 2 経路を `ApplyTitle()` に集約（結果がズレないように）
- [x] SPEC §4 / README（日英）を「照合は元の並びで行う」に更新
- [x] ビルド確認（警告 0）
- [x] 動作確認 — 実機スクショ: 3 タイルとも左端がプロジェクト名で揃い、
      🔵作業中 / 🔑承認 のバッジ・セッション時計・明滅リングがいずれも維持されることを確認
      （＝ `EndsWith` 照合が生きている証拠）
- [x] バージョン 0.15.0 ＋ コミット ＋ `git tag v0.15.0` ＋ publish 常駐入れ替え ＋ push 済み
