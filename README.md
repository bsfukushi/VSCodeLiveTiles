# VSCode Live Tiles

<img src="icon.png" align="right" width="120" alt="VSCode Live Tiles icon">

[English README](README.en.md)

サブモニターに VSCode ウィンドウのライブ縮小表示をタイル状に並べ、
クリックするとメインモニターへ移動して最大化するランチャーウィジェット。

タスクバーのホバープレビューと同じ **DWM Thumbnail API** を使うため、GPU 合成で
リアルタイム・低負荷。

## 開発動機

VSCode を複数ウィンドウで立ち上げ、それぞれで Claude Code を並列実行していると、
どれかが承認待ちや質問で止まっていたり、とっくに作業完了しているのに気づかず、
放置している時間が積み重なっていく。

ウィンドウを切り替えて回れば分かるが、それ自体が手間で、作業中のウィンドウから
離れたくない。そこで **全ウィンドウの「今の状態」をサブモニターに常時見えるように**して、
手が空いたセッションにワンクリックで飛べるようにした。止まっている時間をできるだけ
短くするためのツール。

## セキュリティ・プライバシー

常駐させっぱなしでも安心できるよう、監査しやすい作りにしている。

- **ネットワーク通信ゼロ** — 通信コードがそもそも存在しない
- **テレメトリゼロ** — 何も収集しないし、どこにも送らない
- **NuGet 依存ゼロ** — .NET 標準ライブラリのみ。Win32 API は自前 P/Invoke
- **データに対して読み取り専用** — ウィジェットは events.jsonl を読むだけ。
  書くのは同梱フックだけで、記録するのは状態判定に必要な最小限
  （ts / type / sessionId / projectName / cwd）。**プロンプトやツールの入出力は一切書かない**
- ログはローカルのみ（`%LOCALAPPDATA%\VSCodeLiveTiles\logs\`、7 日で自動削除）

## リソース使用量（実測）

作者環境での実測（v0.10.0、モニター 2 枚・タイル 4 枚、Windows 11）:

| 項目 | 値 |
|---|---|
| Working Set（framework-dependent） | 起動直後 約 140 MB / 常用中 約 180 MB |
| Working Set（self-contained 単体 exe） | 起動直後 約 243 MB |
| CPU（60 秒サンプル平均） | 0.05%（16 コア換算。1 コア換算でも 1% 未満） |

サムネイル合成は DWM（OS のコンポジタ）側で行われるため、
ウィジェット自身は毎フレームの描画処理を持たない。

## 動作要件

- Windows 10 / 11
- .NET 10 デスクトップランタイム（self-contained 発行の単体 exe なら不要）

## ビルド & 実行

```bash
cd S:/Tools/VSCodeLiveTiles
dotnet build -c Release
dotnet run --project src/VSCodeLiveTiles          # デバッグ起動
# もしくは発行して exe 化（ランタイム導入済みの自分用）
dotnet publish src/VSCodeLiveTiles -c Release -r win-x64 --self-contained false -o publish
# → publish/VSCodeLiveTiles.exe を起動

# 配布用: ランタイム不要の完全単体 exe（約 62MB）
dotnet publish src/VSCodeLiveTiles -c Release -r win-x64 --self-contained true `
  -p:PublishSingleFile=true -p:EnableCompressionInSingleFile=true `
  -p:IncludeNativeLibrariesForSelfExtract=true -o publish-standalone
```

起動すると **サブモニター（最初の非プライマリ）** に全画面で常駐し、開いている
VSCode ウィンドウをライブタイルで並べる。タイルをクリックすると、その VSCode が
**メインモニター（プライマリ）で最大化・最前面化**される。

終了はウィジェットを閉じる（Alt+F4）。

## 常駐運用（スタートアップ登録）

日常用は `publish/VSCodeLiveTiles.exe` を使い、ログオン時に自動起動する。
スタートアップフォルダ（`shell:startup`）に `VSCode Live Tiles.lnk` を配置済み。

- **開発ビルド（`bin/`）と常駐 exe（`publish/`）は別物**なので、常駐中でも
  `dotnet build` / `dotnet run` がロックされない（KNOWLEDGE K-2 の回避）
- **更新手順**: ウィジェットを閉じる（Alt+F4）→ `dotnet publish ...`（上記コマンド）→ exe を再起動
- 常駐をやめるにはスタートアップの `.lnk` を削除するだけ

## 設定（`appsettings.json`）

| キー | 既定 | 意味 |
|---|---|---|
| `targetProcessNames` | `["Code"]` | 対象プロセス名（拡張子なし）。増やせば他アプリも対象に |
| `widgetMonitorIndex` | `null` | ウィジェットを載せるモニター。`null`=最初の非プライマリ。番号で明示指定可 |
| `targetMonitorIndex` | `null` | クリックで最大化する先。`null`=プライマリ。番号で明示指定可 |
| `refreshIntervalMs` | `1500` | WinEvent の保険ポーリング間隔。`0` で無効 |
| `captionSuffixesToStrip` | `[" - Visual Studio Code", …]` | キャプションから除去する末尾 |

モニター番号は `EnumDisplayMonitors` の列挙順（0 始まり）。狙いと違う画面に出たら
`widgetMonitorIndex` / `targetMonitorIndex` を数値で指定する。

## CC 状態バッジ（v0.6〜）

同梱の Claude Code フック（`hooks/append-event.mjs`）が追記する
`~/.vscode-live-tiles/events.jsonl` を tail し、各タイルに CC の状態を表示する。

### セットアップ（バッジ機能を使う場合のみ）

```bash
node hooks/install.mjs           # ~/.claude/settings.json にフックを登録
node hooks/install.mjs --dry-run # 書き込まずに変更内容を確認
```

- 登録前に `settings.json.vscode-live-tiles.bak` へ自動バックアップする。
  既存のフック（通知音など）には触らず、`append-event.mjs` の登録だけを置き換える（再実行しても重複しない）
- 登録後はウィジェットを再起動する（CC 側は即時〜次のセッション開始で反映される）
- フックは素の Node.js 単体（依存ゼロ）。どんな失敗でも exit 0 で、Claude Code 本体を止めない
- events.jsonl は 5MB でローテーション（`events.jsonl.1` に 1 世代退避）。
  記録するのは状態判定に必要な最小限（ts / type / sessionId / projectName / cwd）だけで、
  プロンプトやツールの入出力は一切書かない

| バッジ | 状態 | 表示 |
|---|---|---|
| ❓ 質問（黄） | AskUserQuestion で停止中 | 経過時間付き＋枠が明滅 |
| 🔑 承認（橙） | 承認ダイアログ・通知で停止中 | 経過時間付き＋枠が明滅 |
| ✔ 完了（緑） | 応答完了（次の入力まで残る） | バッジのみ |
| ● 作業中（青） | 実行中 | バッジのみ |

- タイルとの対応付けは VSCode 元タイトル（`ファイル名 - フォルダ名`）が `cwd` の
  フォルダ名で終わるかで判定。帯の表示は `フォルダ名 - ファイル名` に並べ替えているが、
  照合は元の並びで行うので影響しない。マッチしないタイルはバッジなし（誤表示より無表示）
- 明滅するのは**まだ見に行っていない待ち**だけ。そのウィンドウを前面に出すと明滅は止まる。
  CC には「承認した瞬間」に発火するフックがなく、承認待ちの解除はツール完了まで遅れるため
- `stop` は完了とは限らない。サブエージェントや `run_in_background` の Bash が
  残っていれば「作業中」のまま（`background_tasks` で判定）
- `~/.vscode-live-tiles/` が無い環境（フック未導入）ではバッジ機能だけ静かに無効化される（他機能に影響なし）
- ウィジェットは読み取り専用（events.jsonl に書くのはフックだけ）。
  パスは環境変数 `VSCODE_LIVE_TILES_EVENTS_FILE` で上書き可（フック・ウィジェット共通、homedir 配下のみ）

## セッション時計（v0.7〜）

キャプション帯に `11:37 ▸ 5:23`（開始時刻 ▸ 経過 h:mm）を表示する。

- 開始時刻は `session_start` の ts。`resume` で再発火しても**最初に観測した方**を採る
- 状態の再構築は末尾 512KB だが、開始時刻はそこに無いことが多いため
  起動時に events.jsonl 全体を 1 度だけ走査して `session_start` 行だけ拾う
  （実測 24MB / 64ms。JSON パースは該当行のみ）
- ローテーションで `session_start` が流れたセッションは時計を出さない（推測しない）

## 設計メモ

- DWM サムネイルは登録先ウィンドウの**描画内容の上**に合成されるため、キャプション/枠は
  サムネイル領域の外（タイル下段の帯）に置いている
- サムネイルは入力を奪わないので、クリックはサムネイル子ウィンドウの `WM_LBUTTONUP` で拾う
- 最小化中のウィンドウはサムネイルが空になるため、タイルを純 WPF のプレースホルダに切替
- タブ切替（タイトル変化）ではサムネイルを作り直さず、キャプションのみ更新（ちらつき防止）
- Per-Monitor v2 DPI 対応。座標は物理ピクセルで扱う

## スコープ

VSCode 限定・自動グリッド・クリックで移動＋最大化（v0.4）、CC 状態バッジ（v0.6）、
セッション時計（v0.7）までを実装済み。
他アプリ対応は `targetProcessNames` を増やせば動く作りにしてある。

## ライセンス

[MIT License](LICENSE)
